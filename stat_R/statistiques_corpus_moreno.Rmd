---
title: "Statistiques sur les entités nommées de lieux - corpus Moreno"
...
```{r}
d <- read.csv('Moreno_NbPlaces.csv', encoding = "UTF-8") 
```
```{r}
summary(d$Occurrences) # Permet d'afficher les quartiles, la moyenne et la médiane
```
En moyenne, un même lieu est présent 6 fois dans le corpus. 
Seul 25% des lieux sont cités plus de 4 fois. 
La moitité des lieux est cité moins de 2 fois. 

```{r}
f <- read.csv('nerList_Moreno_enriched.csv', encoding = "UTF-8")
```

Le nombre d'occurences des lieux apparaissant dans le corpus Moreno ainsi que la fréquence correspondante

```{r}
tab1 <- table(f$normalized_name) #création d'un nouvel objet uniquement les données sur la colonne normalized_name
freq(sort(tab1, decreasing = TRUE)) #permet d'avoir les fréquences classées par ordre décroissant 
```

Graphique simple de la répartition des occurences dans le corpus Moreno. 
```{r}
barplot(sort(tab1), ylim = c(0,250), col="olivedrab3")
```
...
barplot(tab10, ylab="Nombre d'occurences", col=colors, ylim=c(0,250), las=2, cex.names = 0.7)
